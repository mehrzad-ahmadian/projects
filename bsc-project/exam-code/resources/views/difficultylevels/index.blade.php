...
@section('content')
	<div class="uk-grid" style="margin-bottom: 25px;">
		<div class="uk-width-1-2">
			<a href="{{ URL::to('difficultylevels/new') }}" class="md-btn md-btn-primary md-btn-wave-light waves-effect waves-button waves-light" href="form.html">جدید</a>
		</div>
					...
					@foreach ($records as $record)
						<tr>
							...
							<td class="uk-text-center">
								<a href="{{ URL::to('difficultylevels/edit') }}/{{ $record->id }}" class="md-btn md-btn-flat md-btn-wave waves-effect waves-button" style="padding: 0; min-width: 40px;"><i class="md-icon material-icons">&#xE254;</i></a>
								<form action="{{ URL::to('difficultylevels') }}/{{ $record->id }}" method="POST" id="deleteForm{{ $record->id }}" style="display: inline;">
						            {{ csrf_field() }}
						            {{ method_field('DELETE') }}
